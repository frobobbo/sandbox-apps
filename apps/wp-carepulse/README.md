...
- Normalize bare site domains, mixed-case hosts, and explicit default ports (`:443` for HTTPS and `:80` for HTTP) so duplicate entries do not split history and both automatic and manual reports use the canonical URL.
...
- Recommend enabling HTTPS for plain-HTTP sites instead of misreporting that a nonexistent certificate needs renewal.
